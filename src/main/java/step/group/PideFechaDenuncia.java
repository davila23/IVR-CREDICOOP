...
import step.StepValidateDateDenuncia;
import step.group.StepGroupFactory.StepGroupType;
import workflow.Task;

public class PideFechaDenuncia implements StepGroup {
	...
	private StepValidateDateDenuncia validaFecha;
	...
	public PideFechaDenuncia() {
		super();

		GroupType = StepGroupType.pideFechaDenuncia;
		...
		stepAudioFecha.setPlayMaxDigits(8);
		...
		stepDiceAnio = (StepSayNumber) StepFactory.createStep(
				StepType.SayNumber, UUID.randomUUID());
		stepDiceAnio.setStepDescription("SAYNUMBER => ANIO");
		...
		validaFecha = (StepValidateDateDenuncia) StepFactory.createStep(
				StepType.ValidateDateDenuncia, UUID.randomUUID());
